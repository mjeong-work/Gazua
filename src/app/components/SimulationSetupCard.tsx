...
export default function SimulationSetupCard({ simulation, expanded, onToggle }: Props) {
  ...
  return (
    <div className="border border-gray-200 rounded-2xl overflow-hidden">
      ...
      >
        <span className="font-semibold text-gray-900">Simulation Setup</span>
        {expanded ? (
      ...
      {/* Collapsed summary */}
      {!expanded && (
        <div className="px-5 pb-4 flex flex-wrap gap-x-6 gap-y-1 text-sm text-gray-600 border-t border-gray-100">
          <span>
            <span className="text-gray-400 mr-1">Period:</span>
            {formatDateRange(simulation.startDate, simulation.endDate)}
          </span>
          <span>
            <span className="text-gray-400 mr-1">Holdings:</span>
            {holdingSummary}{cashLabel}
          </span>
          <span>
            <span className="text-gray-400 mr-1">Capital:</span>
        ...
        <div className="border-t border-gray-100 px-5 py-5 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-gray-500 mb-1">Simulation Title</label>
              <input
                type="text"
                value={form.title}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Holdings</span>
              <button
                onClick={addHolding}
              ...
              {form.holdings.map((h, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center">
                  <input
                    ...
                    className="col-span-3 border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-violet-300"
                  />
                  <input
                    type="number"
                    placeholder="30000"
                    ...
                    className="col-span-4 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"
                  />
                  <input
                    type="number"
                    placeholder="25"
