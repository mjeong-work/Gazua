...
export default function ReelCommentsAdjacentPanel({
  ...
  return (
    <motion.div
      ...
      aria-label="Comments"
    >
      <div className={`flex items-center justify-between px-5 py-4 border-b flex-shrink-0 ${dark ? 'border-white/10' : 'border-neutral-100'}`}>
        <h2 className="text-base font-bold">Comments</h2>
        <div className="flex items-center gap-3">
          <span className={`text-sm font-medium ${dark ? 'text-white/40' : 'text-neutral-400'}`}>{commentCount}</span>
          <button
            onClick={onClose}
            aria-label="Close comments"
            className={`p-1 rounded-full transition-colors ${dark ? 'hover:bg-white/10' : 'hover:bg-neutral-100'}`}
          >
