...
export default function PastSimulationsList({ simulations, selectedId, onSelect }: Props) {
  ...
  return (
    <div className="border border-neutral-200 rounded-2xl overflow-hidden">
      ...
      >
        <span className="font-semibold text-neutral-900">Past Simulations</span>
        {collapsed ? (
      ...
      {!collapsed && (
        <div className="divide-y divide-gray-100 border-t border-neutral-100">
          {simulations.map(sim => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-5 py-4 transition-colors hover:bg-neutral-50 ${
                  ...
                  <div className="flex items-start gap-3 min-w-0">
                    <div
                      className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${
                        isSelected ? 'bg-violet-500' : 'bg-neutral-300'
                      }`}
                    />
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold truncate ${isSelected ? 'text-violet-700' : 'text-neutral-800'}`}>
                        {sim.title}
                      </p>
