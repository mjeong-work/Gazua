...
export default function AIDisclaimerCard({ compact = false, confidence, className = '' }: AIDisclaimerCardProps) {
  if (compact) {
    return (
      <p className={`flex items-center gap-1.5 text-[10px] text-gray-400 leading-tight ${className}`}>
  ...
  return (
    <div className={`flex items-start gap-3 bg-gray-50 border border-gray-100 rounded-xl p-4 ${className}`}>
      <Sparkles className="w-4 h-4 text-gray-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
      <div>
